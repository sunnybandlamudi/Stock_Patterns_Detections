...
def getMaxMin( STOCK , pmx = False ):
    ...
    low = df['CH_TRADE_HIGH_PRICE'][::]
    return  list(zip(x,y , high ,low))
...
def rectangel(mapping):
    ...
    current = mapping[0]
    percent = 0.5;
    lowPercent = getPercent(current[CLOSE],-percent);
    highPercent = getPercent(current[CLOSE],percent);
    count = 1;
    total = [*localMax,*localMin];
    total.sort(key= lambda item:datetime.strptime(item[0],'%Y-%m-%d'),reverse=True)
    line = [];

    for i in range(len(total)):
        if (
                # lowPercent < localMax[i][HIGH] < highPercent or
                # lowPercent < localMax[i][HIGH]+getPercent(localMax[i][HIGH] , percent) < highPercent or
                # lowPercent < localMax[i][HIGH]+getPercent(localMax[i][HIGH] , -percent) < highPercent or
                lowPercent < total[i][CLOSE] < highPercent or
                lowPercent < total[i][CLOSE] + getPercent(current[CLOSE], percent) < highPercent or
                lowPercent < total[i][CLOSE] + getPercent(current[CLOSE], -percent) < highPercent 
                ):
            line.append(total[i])    
    return (line); 
def getNRone():
